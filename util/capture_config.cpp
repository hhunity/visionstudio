...
capture_config capture_config::load(const std::string& json_path) {
    ...
    // Network/HTTP client settings come from "capture_client" section.
    if (root.contains("capture_client")) {
        const auto& c = root["capture_client"];
    ...
    // App-level capture settings come from "capture" section.
...
void capture_config::save(const std::string& json_path,
    ...
    // Preserve other "capture" keys; only overwrite config file paths.
    if (!root.contains("capture") || !root["capture"].is_object())
        root["capture"] = nlohmann::json::object();
